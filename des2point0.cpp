...
string hex_conversion(string de_msg,string conv_table[23],string hex[23],int key_len){
	int len=4;
   ...
   while(round>0){
   	cout<<"\nRound:"<<i+1<<endl;
    	...
    	if(encrypt==false){
    		...
    		cout<<"\nkey["<<round<<"]:\n"<<hex_conversion(fe_key,conv_table,hex,48)<<endl;
      	}
      	else{
      		fe_key="";
	...
	string xored_message=xored_msg(ep_msg,fe_key);
    	...
    	string L1=Ro;string left_thing=hex_conversion(L1,conv_table,hex,32);
    	cout<<"L["<<i+1<<"]:"<<left_thing<<endl;
	
    	string R1=xored_msg(Lo,permuted_msg);string right_thing=hex_conversion(R1,conv_table,hex,32);
    	cout<<"R["<<i+1<<"]:"<<right_thing<<endl;
     ...
     string hex_encrypted_msg=hex_conversion(des_encrypted_msg,conv_table,hex,64);
	...
	if(tolower(ch)=='y'){
		...
		if(tolower(inp)=='y'){
			...
			while(1){
				 ...
				 cout<<"Enter the  key(all upper case,type quit to quit):";
				 cin>>key1;
				 
				 for(int i=0;i<key1.length();i++){
				int find=0;
				while(1){
					if(key1[i]==characters[find])break;
					find++;
				}
				key=key+ascii_table[find];
			}
